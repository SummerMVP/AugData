...
class SRNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        res = torch.add(res, bn)
        # print(res.shape)
        # np.save('/data2/mingzhihu/code/SRNet/pytorch实现/dalib/'+str(time.time())+'SRNet-75-100.npy', res.cpu().detach().numpy())
        # Layer 8
        ...
        bn = self.bn122(conv2)
        # print("L12:",res.shape)
        avgp = torch.mean(bn, dim=(2, 3), keepdim=True)#全局平均池化
        # fully connected
...
def srnet_delfc():
    model = SRNet()
    temp = torch.load(r'E:\zl\srnet-pytorch\final_model\model_200.pth', map_location='cuda:0')
